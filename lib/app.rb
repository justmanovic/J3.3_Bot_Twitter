...
def hasard(taille_tab)
  return rand(taille_tab) 
end

def contact_nb_random(tab,nb=1)
  nb.times do
    client.update("Hello, #{tab[hasard(tab.length)]} merci pour ce que vous faites <3 ! #bonjour_monde @the_hacking_pro")
  end
end

# contact_nb_random(journalists, 5)  >>>>>> Appel de la fonction !  

# client.search("from:JMJancovici", result_type:"recent").take(10).collect do |tweet|
#   puts puts "#{tweet.user.screen_name}: #{tweet.text}"
# end

# Pour liker un tweet 
# tweet = client.search("from:JMJancovici").take(25)[1]
# client.favorite(tweet) 

# Pour retweeter un tweet
# client.retweet(tweet)

# pour liker tous les tweets avec le #bonjour_momnde
# client.search("#bonjour_monde", result_type:"recent").take(25).collect do |tweet|
#   client.favorite(tweet.id) 
# end

# pour follow tous les tweets avec le #bonjour_momnde
...
topic = ["zidane"]
client.filter(track: topic.join(",")) do |object|
  if object.is_a?(Twitter::Tweet)
    client2.follow(object.user)
    puts "#{object.user.screen_name} :: #{object.text}"
    sleep(2)
  end
end

# client2.update("hello la compagnie")
...
#   when Twitter::Tweet
#     puts 
#     if ["#bonjour_monde"].include?(object)
#        
#       client.favorite(tweet) 
#     end
#   end
